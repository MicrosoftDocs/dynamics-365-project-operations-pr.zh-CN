...
## <a name="using-entry-journals-to-record-costs"></a>使用条目日记帐记录成本

在 PSA 中，可使用条目日记帐记录材料、费用、时间、支出或税务交易分类的成本或收入。 日记帐有标题、行和 **确认** 操作。 下面是一些可能需要使用日记帐的场景：
...
> [!IMPORTANT]
> 用户只有完全了解“实际值”对项目会计的影响，才能使用条目日记帐创建实际值。 这是因为此应用程序不会验证日记帐行类型，也不会验证在日记帐行上输入的相关定价。 由于此日记帐类型会造成一定的影响，因此，对于谁有权创建条目日记帐，应该十分谨慎。     
