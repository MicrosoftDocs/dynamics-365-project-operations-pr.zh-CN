...
4. 如果不使用用户名为 **spencerl** 的 Spencer Low，您需要更新其他文件。 打开 **DemoDataPreImportConfig.xml** 文件，然后找到 **userstocreateandconfigure** 标记。 使用 Spencer Low 用户的用户名更新**\<登录\>** 标记。 其他详细信息，请参阅[技术说明](#technical-notes)。
...
4. 如果您的 "Spencer Low" 全名用户的用户 ID 与 **"spencerl"** 不同，则需要更新其他文件。 打开 **DemoDataPreImportConfig.xml**，找到 **userstocreateandconfigure** 标记。 将 **\<登录\>** 标记更新为 loginId（区分大小写）。 
