...
### <a name="fieldbasetype-types"></a>fieldBaseType（类型）

**TsTimesheetCustom** 对象的 **FieldBaseType** 属性决定应用中显示的字段类型。 支持以下 **类型** 值。
...
可使用此属性将实数值的格式设置为货币。 仅当 **fieldBaseType** 值为 **实数** 时，这种方法才适用。
...
此属性指定字符串字段的最大长度。 仅当 **fieldBaseType** 设置为 **字符串** 时才适用。

### <a name="numberofdecimals-int"></a>numberOfDecimals（整数）

此属性指定为实数字段显示的小数位数。 仅当 **fieldBaseType** 设置为 **实数** 时才适用。

### <a name="ordersequence-int"></a>orderSequence（整数）

此属性控制当指定了多个自定义字段时自定义字段在应用中的显示顺序。 编号较小的字段先显示。

### <a name="booleanvalue-boolean"></a>booleanValue（布尔）

对于类型为 **布尔** 的字段，此属性在服务器与应用之间传递布尔值类型的字段值。
...
### <a name="intvalue-int"></a>intValue（整数）

对于类型为 **整数** 的字段，此属性在服务器与应用之间传递整数类型的字段值。

### <a name="realvalue-real"></a>realValue（实数）

对于类型为 **实数** 的字段，此属性在服务器与应用之间传递字段的实数值。

### <a name="stringvalue-str"></a>stringValue（字符串）

对于类型为 **字符串** 的字段，此属性在服务器与应用之间传递字符串类型的字段值。 还用于格式设置为货币的 **实数** 类型的字段。 对于这些字段，此属性用于将币种代码传递到应用。

### <a name="datevalue-date"></a>dateValue（日期）

对于类型为 **日期** 的字段，此属性在服务器与应用之间传递日期类型的字段值。
...
> [!NOTE]
> 以下示例将用户选择的 **firstOption** 或 **secondOption** 值作为原始字符串值保存到数据库。 如果数据库字段为 **枚举** 类型的字段，则可以将这些值手动映射到枚举值，然后保存到数据库表中的枚举字段。
