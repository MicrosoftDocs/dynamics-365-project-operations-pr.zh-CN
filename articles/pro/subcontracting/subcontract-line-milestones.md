...
4. 在 **新建分包合同子项里程碑** 页面中，根据下表输入必需值。
    ...
    | 里程碑名称 | 里程碑的名称。 |
    | 描述 | 里程碑的说明。  |
    | 里程碑日期 | 发票自动创建流程应查找此里程碑的状态以考虑为其开票时的日期。 为此分包合同开票时，供应商发票明细中包含此值。 |
    | 数量 | 将向客户开发票的里程碑的金额或值。 为此分包合同开票时，供应商发票明细中包含此值。 |
    | 税款 | 里程碑上应用的税额。 为此分包合同开票时，供应商发票明细中包含此值。 |
    | 税后金额 | 这个只读字段的计算方法为金额 + 税。 为此分包合同开票时，供应商发票明细中包含此值。 |
