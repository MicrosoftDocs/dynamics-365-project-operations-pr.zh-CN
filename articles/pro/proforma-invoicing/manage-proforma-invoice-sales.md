---
title: 管理估价项目账单
description: 本文提供有关如何处理估价项目发票的信息。
author: rumant
ms.date: 04/05/2021
ms.topic: article
...
---
# <a name="manage-a-proforma-project-invoice"></a>管理估价项目账单 
...
对于 **时间和材料发票** 明细，发票明细详细信息在 **发票明细** 页上按 **应计费**、**非应计费** 和 **免费** 进行分组。 **应计费发票明细** 详细信息加总为发票明细总额。 **免费** 和 **非应计费实际值** 不会添加到发票明细总额。
...
| **选择产品** | 这是从源实际值中默认设置的只读字段。 | 当您创建没有支持实际值的新发票明细详细信息时，可以编辑此字段。 |
| **产品** | 这是从源实际值中默认设置的只读字段。 | 当您创建没有支持实际值的新发票明细详细信息时，如果 **选择产品** 字段设置为 **现有产品**，则可以编辑该字段。 |
| **产品名称** | 这是从源实际值中默认设置的只读字段。 | 在从目录中选择产品 ID 的新发票明细详细信息中，此字段设置为产品名称。 对于目录外产品，此字段设置为目录外名称。 |
| **目录外说明** | 这是从源实际值中默认设置的只读字段。 | 当您创建没有支持实际值的新发票明细详细信息时，可以为产品添加目录外说明。 |
| **交易类型** | 默认根据源实际值设置。 一个被锁定无法编辑的只读字段。 | 默认设置为 **已记帐销售额**，在创建没有支持实际值的新 **发票明细详细信息** 时锁定。  |
| **交易分类** | 默认根据源实际值设置。 一个被锁定无法编辑的只读字段。 | 根据用户是选择创建 **时间**、**支出**、**材料** 还是 **费用** 发票明细详细信息进行默认设置，同时还创建没有支持实际值的 **发票明细详细信息**。 已锁定，无法编辑。 |
...
### <a name="create-new-invoice-line-details"></a>创建新发票明细详细信息

在时间和材料发票明细中，您可以创建新的发票明细详细信息。 这些发票明细详细信息不由实际值支持。 在时间和材料发票明细的发票明细中，您可以选择 **新建** 来为合同子项包括的交易类创建新发票明细详细信息。
...
## <a name="product-based-invoice-lines"></a>基于产品的发票明细

在 Project Operations 中，您可以随基于项目的发票明细一起，为不适用于任何项目的产品或为所有项目创建发票明细。 这些发票明细创建为基于产品的合同子项，在标记为可开票后，它们将添加为基于产品的发票明细。

添加基于产品的发票明细之后，将无法对其进行更改。 不过，可以从草稿估价发票中将其删除。
